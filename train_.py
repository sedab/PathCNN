...
root_dir = "/gpfs/scratch/bilals01/brain-kidney-lung/brain-kidney-lungTilesSorted/"
num_classes = 7
tile_dict_path = '/gpfs/scratch/bilals01/brain-kidney-lung/brain-kidney-lung_FileMappingDict.p'
...
for epoch in range(opt.niter+1):
    ...
    while i < len(loaders['train']):
        ...
        if ii % 1000 == 0:
            ...
            os.chdir('/gpfs/scratch/bilals01/test-repo/PathCNN/experiments/')
            data_ = np.column_stack((np.asarray(val_predictions),np.asarray(val_labels)))
            data_.dump(open('experiments/{0}/pred_label_avg_{0}_step_{1}.npy'.format(opt.experiment,str(ii)), 'wb'))
            torch.save(model.state_dict(), 'experiments/{0}/checkpoints/step_{1}.pth'.format(opt.experiment, str(ii)))           
            print('validation scores:')

            os.chdir('/gpfs/scratch/bilals01/test-repo/PathCNN')
